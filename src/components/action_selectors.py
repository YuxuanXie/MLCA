...
class GumbelSoftmax(OneHotCategorical):
    ...
    def __init__(self, logits=None, probs=None, temperature=1):
...
def multinomial_gumbel_entropy(probs=None, logits=None):
    if probs != None:
        assert probs.size(-1) > 1
    else:
        assert logits.size(-1) > 1
    return GumbelSoftmax(probs=probs, logits=logits).entropy()
...
class GumbelSoftmaxMultinomialActionSelector():
    ...
    def select_action(self, agent_probs, avail_actions, t_env, test_mode=False):
        masked_policies = agent_probs.clone()
        ...
        if test_mode and self.test_greedy:
            ...
            picked_actions = GumbelSoftmax(probs=masked_policies).sample()
            picked_actions = th.argmax(picked_actions, dim=-1).long()

        if (th.gather(avail_actions, dim=2, index=picked_actions.unsqueeze(-1)) == 0).sum() > 0:
...
def multinomial_entropy(probs=None, logits=None):
    if probs != None:
        assert probs.size(-1) > 1
    else:
        assert logits.size(-1) > 1
    return Categorical(probs=probs, logits=logits).entropy()
